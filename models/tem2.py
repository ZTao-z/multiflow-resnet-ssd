...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes,resnet18,resnet18_1):
    ...
    def forward(self, x):
        ...
        s = torch.cat((s, s2,s4, s6, s8), 1)
        for k in range(len(self.vgg7)):
            s = self.vgg7[k](s)
        s38 = self.L2Norm2(s)

        s38_max,indices=F.max_pool2d(s38,kernel_size=3,stride=2,padding=1,return_indices=True)
        s38_max=F.max_unpool2d(s38_max,indices,kernel_size=2,stride=2,padding=0)
        s38=s38_max//2+s38
        # sources.append(s)
...
def vgg(cfg, i, batch_norm=False):
    ...
    conv12 = nn.Conv2d(1536, 512, kernel_size=1)
...
def build_net(phase, size=300, num_classes=21):
    ...
    return SSD(phase, size, base_, extras_, head_, num_classes,resnet18(),resnet18())
